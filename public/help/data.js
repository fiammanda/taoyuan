(() => {
  ...
  // src/data/items.ts
  var SEED_ITEMS = CROPS.filter((crop) => crop.seedId !== "ancient_seed" && crop.seedId !== "hanhai_cactus_seed" && crop.seedId !== "hanhai_date_seed").map((crop) => ({
    id: crop.seedId,
    name: `${crop.name}种子`,
    category: "seed",
  ...
  for (const mod of modules) {
    for (const key in mod) {
      if (/^[A-Z_]+$/.test(key)) {
        HelpData[key] = mod[key];
      }
    }
  }
  window.DATA = HelpData;
})();
